<template>
    <div class="wrap">
        ...
        <div>
            <p>购买顶部商务功能栏，能够帮助用户快捷进入购物车及会员中心！</p>
            <p>购买后，请进入网站编辑器，在页眉区域右键点击，选择显示顶部商务功能栏。</p>
        </div>
        <div v-if="topBarInfo.fOrderDueTime && topBarInfo.fIsPurchase !== 2" class="deadline">
            当前服务已付费至{{topBarInfo.fOrderDueTime | formatTime}}
        </div>
        <div class="timeChoose">
            <div>购买期限</div>
            <div>
                <div :class="{'active':chooseTimeType === item.fType}" v-for="(item,index) in topBarInfo.priceBeanList"
...
<script>
    export default {
        ...
        created() {
            this.topBarInfo = this.pageData.controlInfo;
            if (this.pageData.type === 4) {
                this.isShowChoose = false;
            } else {
                this.isShowChoose = true;
            }
            for (let i = 0; i < this.topBarInfo.priceBeanList.length; i++) {
                this.timeObj[this.topBarInfo.priceBeanList[i].fType].fPrice = this.topBarInfo.priceBeanList[i].fPrice;
                this.timeObj[this.topBarInfo.priceBeanList[i].fType].fDiscountsType = this.topBarInfo.priceBeanList[i].fDiscountsType;
                this.timeObj[this.topBarInfo.priceBeanList[i].fType].fIsDiscounts = this.topBarInfo.priceBeanList[i].fIsDiscounts;
                this.timeObj[this.topBarInfo.priceBeanList[i].fType].fDiscountsPercent = this.topBarInfo.priceBeanList[i].fDiscountsPercent;
                if (this.topBarInfo.priceBeanList[i].fControlHotSale === 1) {
                    this.chooseTimeType = this.topBarInfo.priceBeanList[i].fType;
                }
            }
        },
        computed: {

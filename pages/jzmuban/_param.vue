...
<script>
    ...
    export default {
        ...
        async asyncData({app,params}) {
            ...
            let bigCategoryNameObj = {};
            let smallCategoryNameObj = {};
            let templateBigCategory = requestUtil.validateData(await requestUtil.getData(interfaceApi.getTemplateBigCategory, null));
            let categoryIdArr = params.param.split("_");
            for(let i = 0; i < templateBigCategory.length; i++){
                bigCategoryNameObj[templateBigCategory[i].fId] = templateBigCategory[i].fCategoryName;
            }
            obj.fLargeCategoryName = bigCategoryNameObj[categoryIdArr[0]];
            templateBigCategoryId = categoryIdArr[0];
            templateSmallCategory = requestUtil.validateData(await requestUtil.getData(interfaceApi.getTemplateSmallCategory, {categoryId:categoryIdArr[0]}));
            if(categoryIdArr.length > 1){//有二级类目的时候
                for(let j = 0; j < templateSmallCategory.length; j++){
                    smallCategoryNameObj[templateSmallCategory[j].fId] = templateSmallCategory[j].fCategoryName;
                }
                obj.fSmallCategoryName = smallCategoryNameObj[categoryIdArr[1]];
                templateSmallCategoryId = categoryIdArr[1];
            }
            ...
            return {
                ...
                bigCategoryNameObj:bigCategoryNameObj,
                smallCategoryNameObj:smallCategoryNameObj
            };
        },
        head() {
            return {

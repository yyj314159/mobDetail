      ...
      <g>
        ...
        <main>
            <div>
                ...
                <bindPhone v-if="isBindPhone" :obj="obj"></bindPhone>
                <notCanLogin v-if="isNotCanLogin"></notCanLogin>
            </div>
        </main>
        <footer>
            <foot></foot>
...
<script>
    import login from '~/components/login/loginWay/login';
    import foot from '~/components/public/foot/foot1';
    import wxLogin from '~/components/login/loginWay/wxlogin';
    import bindPhone from '~/components/login/bindPhone/bindPhone';
    import notCanLogin from '~/components/login/notCanLogin';
    ...
    export default {
        ...
        head() {
            return {
                'title': '族蚂网登录',
                meta: [
                    {name: 'keywords', content: '登录族蚂网，族蚂网登录，会员登录，注册会员，注册族蚂网，族蚂网账号，族蚂账号，建账号，忘记密码，创建账户，会员账户，商家账户，商家入驻，入驻族蚂网'},
                    {
                        name: 'description',
                        content: '族蚂网登录页，微信快捷登录，使用微信扫描二维码，安全登录，QQ快捷登录，忘记密码，找回密码，设置密码，注册，免密快捷登录，账户密码登录_建站，制作小程序，公众号页面，H5微商城，微传单，请用族蚂网'
                    }
                ],
                script: [
                    {
        ...
        mounted() {
            $.ajax({
                url:this.interfaceApi.getLoginStatus,
                async:false,
                success:(res)=>{
                    if(res.status === 0){
                        if(res.data){
        ...
        methods: {
            wxInit() {
                $.ajax({
                    ...
                    success: (res) => {
                        if (res.status === 0) {
                            this.wxData = JSON.parse(res.data);
                            this.obj.type = "wx";
                            this.obj.userName = this.wxData.nickname;
                            this.obj.headImg = this.wxData.headimgurl;
                            this.obj.userOpenId = this.wxData.unionid;
                            this.obj.wxOpenId = this.wxData.openid;
                        }
                    }
                });
                if (this.wxData) {
                    $.ajax({
                        url: '/newLogin',
                        data: {
                            userName: this.wxData.openid,
                            userPassword: this.wxData.unionid,
                            type: 3
                        },
                        success: (res) => {
                            if (res.status === 2) {
                                ...
                                this.isNotCanLogin = false;
                            }else if(res.status === 4){
                                this.isWxLogin = false;
                                this.isLogin = false;
                                this.isBindPhone = false;
                                this.isNotCanLogin = true;
                            } else {
            ...
            qqInit() {
                $.ajax({
                    ...
                    data: {
                        code: this.$route.hash.split("#")[1].split("&")[0].split("=")[1],
                    },
                    async: false,
                    success: (res) => {
                        console.log(res);
                        if (res.status === 0) {
                            this.obj.type = "qq";
                            this.obj.userName = res.data.nickname;
                            this.obj.headImg = res.data.figureurl_2;
                            this.obj.qqOpenId = res.message;
                        }
                    }
                });
                if (this.obj.qqOpenId) {
                    $.ajax({
                        url: '/newLogin',
                        data: {
                            userName: this.obj.qqOpenId,
                            type: 4
                        },
                        success: (res) => {
                            console.log(res);
